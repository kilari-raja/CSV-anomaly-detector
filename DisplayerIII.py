...
def print_not_state_code(filesname,start,end,datum,print_array):    
    ...
    for i in range(1,len(datum)):       
        row_no_in_original_file += 1                        
        if (datum[i][start] not in states) or (datum[i][start] not in lower_states) or (
         (datum[i][start] not in upper_states) or (datum[i][start] != " ") ) or (
          (datum[i][start] != "") ):
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,
                                              row_no_in_original_file,func_count,i,
...
def print_zip_code(filesname,start,end,datum,print_array):  
    ...
    for i in range(1,len(datum)):       
        ...
        find_space = re.findall(pattern_space,datum[i][start])
        find_dot = re.findall(pattern_dot,datum[i][start])
        if len(find_phone) == 1 and len(find_integer) < 10 and not find_string and (not 
...
def print_improper_zip_code(filesname,start,end,datum,print_array):  
    ...
    for i in range(1,len(datum)):        
        ...
        if len(find_phone) == 1 and len(find_integer) < 10 and not find_string and (not 
                ...
                for j in range(0,len(zipcodes[y])):
                    for k in range(0,len(datum[i])):
                        if (zipcodes[y][j]==datum[i][k]) :
                            ...
                            pass
                # print "present_in_row outide  222 is",present_in_row
                if present_in_row == 0:
            ...
            find_dot and not find_string and not find_space ):
            # print "find_integer is",find_integer
            # print "len(find_integer) is",len(find_integer)47906
            #print "datum[i][start] is",datum[i][start]
            present_in_row=0
            c = "0" + datum[i][start]
            if zipcodes[c]:
                ...
                #print "present_in_row outide  is",present_in_row
                if present_in_row == 0: 
                    print "datum[i][start] 111 is",datum[i][start]
                    #print "present_in_row inside  is",present_in_row
                    defective_rows += 1                    
                    funct_count = print_in_improperTxt(datum,start,defective_rows,
                                                      row_no_in_original_file,func_count,i,
