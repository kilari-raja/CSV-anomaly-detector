...
def print_uppercase_entries(filesname,start,end,datum,print_array): 
    ...
    for i in range(1,len(datum)):        
        ...
        if find_uppercase :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,29,print_array)
    return print_array
def print_empty_entries(filesname,start,end,datum,print_array): 
    ...
    for i in range(1,len(datum)):
        ...
        if find_empty or find_no_entry :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,6,print_array)
    return print_array
def print_string_entries(filesname,start,end,datum,print_array):    
    ...
    for i in range(1,len(datum)):       
        ...
        if find_string :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,26,print_array)
    return print_array
...
def print_string_with_integer_and_space_entries(filesname,start,end,datum,print_array): 
    ...
    for i in range(1,len(datum)):
        ...
        if find_string and find_integer and find_space :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,28,print_array)
    return print_array  
...
def print_integer_only_entries(filesname,start,end,datum,print_array):      
    ...
    for i in range(1,len(datum)):       
        ...
        if find_integer and not find_string and len(find_integer)==len(datum[i][start]) :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,13,print_array)
    return print_array  
def print_improper_decimal_integers(filesname,start,end,datum,print_array): 
    ...
    for i in range(1,len(datum)):       
        ...
        if find_integer and not find_string and len(decimal_integer_lengths) > 0  and len(find_dot) != counter_decimal_integer.most_common(1)[0][0]:
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,9,print_array)    
    return print_array
...
def print_email_entries(filesname,start,end,datum,print_array): 
    ...
    for i in range(1,len(datum)):       
        ...
        if find_email and find_dot and not find_space :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,5,print_array)    
    return print_array
def print_website_entries(filesname,start,end,datum,print_array):   
    ...
    for i in range(1,len(datum)):       
        ...
        if find_website and find_dot:
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,32,print_array)    
    return print_array  
...
def print_special_characters_phone(filesname,start,end,datum,print_array):  
    ...
    for i in range(1,len(datum)):       
        ...
        if find_special_characters_phone :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,24,print_array)    
    return print_array
def print_special_characters_website(filesname,start,end,datum,print_array):    
    ...
    for i in range(1,len(datum)):       
        ...
        if find_special_characters_website :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,25,print_array)    
    return print_array  
...
def print_no_dots(filesname,start,end,datum,print_array):   
    ...
    for i in range(1,len(datum)):       
        ...
        if (find_string and not find_dot) :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,18,print_array)    
    return print_array
def print_pure_integer_not_zipcode(filesname,start,end,datum,print_array):  
    ...
    for i in range(1,len(datum)):       
        ...
        if find_integer and not find_phone and not find_zipcode_without_hyphen and not find_zipcode_four_digits and not find_string and not find_dot:
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,12,print_array)    
    return print_array
def print_integer_more_than_string(filesname,start,end,datum,print_array):  
    ...
    for i in range(1,len(datum)):       
        ...
        if (len(find_integer) > len(find_string) and not find_website) :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,15,print_array)    
    return print_array
def print_string_more_than_integer(filesname,start,end,datum,print_array):  
    ...
    for i in range(1,len(datum)):       
        ...
        if (len(find_string) > len(find_integer)) :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,16,print_array)    
    return print_array
def print_not_state_code(filesname,start,end,datum,print_array):    
    ...
    for i in range(1,len(datum)):       
        ...
        if (datum[i][start] not in states) or (datum[i][start] not in lower_states) or (datum[i][start] not in upper_states) or (datum[i][start] != " ") or (datum[i][start] != "") :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,17,print_array)    
    return print_array
def print_symbols(filesname,start,end,datum,print_array):
    ...
    for i in range(1,len(datum)):       
        ...
        if find_string and find_at_the_rate and not find_dot :          
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,34,print_array)
        if find_string and find_hashtag and not find_space :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,1,print_array)
        ...
        if (find_string and find_open_parantheses) or(find_string and find_close_paranthses):
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,19,print_array)
        ...
        if find_integer and find_at_the_rate and find_dot and not find_string:
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,35,print_array)
        if (find_integer and find_open_parantheses) or (find_integer and find_close_paranthses):
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,14,print_array)    
    return print_array
def print_decimal_values(filesname,start,end,datum,print_array):        
    ...
    for i in range(1,len(datum)):       
        ...
        if find_integer and not find_zipcode_without_hyphen and not find_zipcode_four_digits and not find_string and find_dot:
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,2,print_array)    
    return print_array
    ...
    for i in range(1,len(datum)):       
        ...
        if (datum[i][start] in state_code_array)and (datum[i][start] not in states_and_cities) :   
            for j in range(i+1,len(datum)):
                ...
                if key[0] == key[1] :
                    defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,30,print_array)
    return print_array
def print_state_code_lowercase(filesname,start,end,datum,print_array):  
    ...
    for i in range(1,len(datum)):
        ...
        if (datum[i][start].upper() in state_code_array)and find_lowercase:
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,31,print_array)    
    return print_array
    ...
    if len(duplicate_emails) > 0 :
        ...
        for x in range(0,len(duplicate_emails)) :
            if duplicate_emails[x] in str(row) :
                defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,4,print_array)    
    return print_array
def print_improper_email_entries(filesname,start,end,datum,print_array):    
    ...
    for i in range(1,len(datum)):
        ...
        if len(find_at_the_rate) >=2 :
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,37,print_array)    
        ...
        if ((find_string and find_integer and not find_empty and not find_no_entry  and 
            ...
            (find_string and not find_integer and not find_empty and
             find_dot and not find_email and not find_website and not
              find_no_entry)):
            defective_rows += 1
            funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,3,print_array)    
    return print_array
        ...
        if len(find_phone) == 1 and len(find_integer) < 10 and not find_string and not find_hashtag and not find_pattern_phone_parantheses and not find_slash and not find_space :
            ...
            for j in range(0,len(zipcodes)):
                if datum[i][start] == zipcodes[j] :
                    defective_rows += 1
                    funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,20,print_array)
        elif len(find_integer) == 4 and not find_phone and not find_hashtag and not find_dot and not find_string and not find_space :
            c = "0" + datum[i][start]
            for j in range(0,len(zipcodes)):
                if c == zipcodes[j] :
                    defective_rows += 1
                    funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,7,print_array)    
    return print_array
...
def print_improper_zip_code(filesname,start,end,datum,print_array):  
    ...
    for i in range(1,len(datum)):        
        ...
        find_dot = re.findall(pattern_dot,datum[i][start])
        if len(find_phone) == 1 and len(find_integer) < 10 and not find_string and not find_hashtag and not find_pattern_phone_parantheses and not find_slash and not find_space :
            print "datum[i][start] is",datum[i][start]
            present_in_row=0
            y = datum[i][start].split("-")[0]
            if zipcodes[y] and len(y)>3:
                for i in range(0,len(zipcodes[y])):
                    for k in range(0,len(datum[i])):
                        if (zipcodes[y][i]==datum[i][k]) :
                            present_in_row+=1
                        else:
                            pass
                print "present_in_row outide  is",present_in_row
                if present_in_row == 0:
                    print "present_in_row inside  is",present_in_row
                    print "y is",y
                    defective_rows += 1
                    print "1"
                    funct_count = print_in_improperTxt(datum,start,defective_rows,row_no_in_original_file,func_count,i,39,print_array)
        elif len(find_integer) == 5 and not find_phone and not find_hashtag and not find_dot and not find_string and not find_space :
            print "datum[i][start] is",datum[i][start]
            present_in_row=0           
            ...
            c = "0" + datum[i][start]
            if zipcodes[c]:
                for i in range(0,len(zipcodes[c])):
                    for k in range(0,len(datum[i])):
                        print "zipcodes[c][i] is",zipcodes[c][i]
                        print "datum[i][k] is",datum[i][k]
                        if (zipcodes[c][i]==datum[i][k]) :
                            present_in_row+=1
                        else:
                            pass
                print "present_in_row outide  is",present_in_row
