...
def observations(filesname,start,end,counter,datum,emailReturn,bdict):
	...
	if((total_email) > (9*(counter - empty))/10):		
		...
		# print "datum is",datum
		returnPrintFunction = print_improper_email_entries(filesname,start,end,datum)		
		returnPrintFunction = print_integer_only_entries(filesname,start,end,datum)		
		returnPrintFunction = print_integer_more_than_string(filesname,start,end,datum)	
	if(empty) < (counter/10) and empty > 0 :
		...
		# returnPrintFunction = print_empty_entries(filesname,start,end,datum)
	...
	if(state_code > (6*(counter - empty))/10):
		...
		returnPrintFunction = print_not_state_code(filesname,start,end,datum)
		returnPrintFunction = print_state_code_lowercase(filesname,start,end,datum)	
	if(total_decimal_integer > (5*counter)/10) :
		...
		returnPrintFunction = print_symbols(filesname,start,end,datum)
		returnPrintFunction = print_string_entries(filesname,start,end,datum)
		returnPrintFunction = print_integer_only_entries(filesname,start,end,datum)
		returnPrintFunction = print_improper_decimal_integers(filesname,start,end,datum)
	if(total_website > 5*(counter-empty)/10 ):
		...
		returnPrintFunction = print_email_entries(filesname,start,end,datum) 			
		returnPrintFunction = print_integer_only_entries(filesname,start,end,datum)		
		returnPrintFunction = print_space_entries(filesname,start,end,datum)
		returnPrintFunction = print_no_dots(filesname,start,end,datum)
		# returnPrintFunction = print_integer_more_than_string(filesname,start,end,datum)
		if special_characters_print == 0 :
			returnPrintFunction = print_special_characters_website(filesname,start,end,datum)
			# global special_characters_print
			special_characters_print+=1
		returnPrintFunction = print_string_only_entries(filesname,start,end,datum)
		# returnPrintFunction = print_uppercase_entries(filesname,start,end,datum)
		if(empty) < (counter/10) and empty > 0 :
	...
	if((total_pure_string) > (9*(counter - empty))/10):
		...
		if(state_code > (6*(counter - empty))/10):
			returnPrintFunction = print_integer_entries(filesname,start,end,datum)
			returnPrintFunction = print_email_entries(filesname,start,end,datum)
			returnPrintFunction = print_symbols(filesname,start,end,datum)
		if(total_email > counter/2):
			...
			returnPrintFunction = print_email_entries(filesname,start,end,datum)
			returnPrintFunction = print_state_code(filesname,start,end,datum)
			if special_characters_print == 0:					
				returnPrintFunction = print_special_characters(filesname,start,end,datum)
				# global special_characters_print
				special_characters_print+=1
			returnPrintFunction = print_hyphen(filesname,start,end,datum)
			returnPrintFunction = print_symbols(filesname,start,end,datum)
			returnPrintFunction = print_integer_entries(filesname,start,end,datum)
			returnPrintFunction = print_string_with_dots_not_email_not_website(filesname,start,end,datum)
			returnPrintFunction = print_email_without_dot(filesname,start,end,datum)
	...
	if((email_without_integer + email_with_integer) > (counter/2)) :
		...
		returnPrintFunction = print_improper_email_entries(filesname,start,end,datum)
		# returnPrintFunction = print_uppercase_entries(filesname,start,end,datum)
		# returnPrintFunction = print_duplicate_email_entries(filesname,start,end,datum)
		if(empty):
			print "\tThere are empty records in this column"
	if (total_zipcode > 0) and (total_zipcode <= (counter/10)):				
			returnPrintFunction = print_zip_code(filesname,start,end,datum)
	if(total_phone_only_integers + total_pure_integer) > (5*counter)/10 :	
		...
		returnPrintFunction = print_string_without_hyphen_entries(filesname,start,end,datum)
		if(total_phone) > (4*counter)/10 :
			if special_characters_print == 0:
				returnPrintFunction = print_special_characters_phone(filesname,start,end,datum)
				# global special_characters_print
				special_characters_print+=1
		else :
			if special_characters_print == 0:
				...
				# global special_characters_print
				special_characters_print+=1
		returnPrintFunction = print_string_with_symbol_at_but_not_email(filesname,start,end,datum)
		returnPrintFunction = print_string_with_parantheses(filesname,start,end,datum)
		returnPrintFunction = print_integer_with_symbol_at_but_not_email(filesname,start,end,datum)
		returnPrintFunction = print_integer_with_symbol_at_and_dot(filesname,start,end,datum)
		# returnPrintFunction = print_integer_with_parantheses(filesname,start,end,datum)
		returnPrintFunction = print_string_with_hashtag_without_space(filesname,start,end,datum)
		returnPrintFunction = print_integer_with_hashtag_without_space(filesname,start,end,datum)
		returnPrintFunction = print_decimal_values(filesname,start,end,datum)
	if(total_phone) > (4*counter)/10 :	
	...
	if(total_zipcode > (counter/2)) :
		...
		if special_characters_print == 0:
			...
			# global special_characters_print
			special_characters_print+=1
		returnPrintFunction = print_pure_integer_not_zipcode(filesname,start,end,datum)
	...
	if(uncertain_entries):
		...
		improper_integer_entries(filesname,start,end,datum)	
	if(returnPrintFunction != 0):
		print "****************************"
